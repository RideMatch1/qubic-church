...
This is separate from the Anna coordinate system conversion.
"""
import json
import hashlib
...
def seed_to_coordinates(seed: str) -> tuple[int, int]:
    ...
    Note: This maps hash bytes directly to matrix indices (row, col).
    To get the corresponding Anna coordinates, use matrix_to_anna(row, col).
    """
...
def run_core_resonance_deep_audit():
    print("[*] Deep Resonance Audit on CORE SECTOR...")
    
    with open(COMMITTEE_JSON, "r") as f:
        committee = json.load(f)
    core_info = committee.get("CORE", {})
    start, end = core_info['id_range']
