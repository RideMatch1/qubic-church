          ...
          {activeTab === 'formula' && (
            <div className="space-y-6">
              <div className="p-6 rounded-xl border border-border bg-card/50 backdrop-blur-sm">
                ...
                <div className="mt-6 p-4 rounded-lg bg-primary/10 font-mono text-sm">
                      ...
                      <div key={idx} className="p-3 rounded-lg bg-muted/30 border border-border">
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {method.xorValues.map((xor) => (
                            <span key={xor} className="px-2 py-0.5 rounded bg-primary/20 text-xs font-mono">
                              XOR {xor}
                            </span>
                          ))}
                        </div>
                  ...
                  {VERIFICATION_CHECKLIST.map((item, idx) => (
                    <div
                      ...
                      className={cn(
                        ...
                        item.verified ? 'bg-green-500/10' : 'bg-yellow-500/10'
                      )}
                    >
                      <div className="flex items-center gap-3">
                        <div className={cn(
                          'w-5 h-5 rounded-full flex items-center justify-center text-xs',
                          item.verified ? 'bg-green-500 text-white' : 'bg-yellow-500 text-black'
                        )}>
                          {item.verified ? '✓' : '?'}
                        </div>
                      ...
                      <span className={cn(
                        'text-sm font-medium',
                        item.confidence >= 95 ? 'text-green-400' :
                        item.confidence >= 70 ? 'text-blue-400' :
                        'text-yellow-400'
                      )}>
                        {item.confidence}%
                      </span>
