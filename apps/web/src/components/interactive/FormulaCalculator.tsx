...
export function FormulaCalculator() {
  ...
  return (
    <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] backdrop-blur-sm overflow-hidden">
      ...
      {/* Formula Display */}
      <div className="mx-6 md:mx-8 mb-8 p-5 md:p-6 backdrop-blur-xl bg-white/[0.03] border border-white/[0.08] text-center">
        <div className="text-2xl md:text-3xl lg:text-4xl font-mono mb-3 leading-relaxed">
          <span className="text-[#D4AF37]/80">{blockHeight}</span>
          <span className="text-white/30"> &times; </span>
          <span className="text-[#D4AF37]/80">{primeValue}&sup2;</span>
          <span className="text-white/30"> + </span>
          <span className="text-[#D4AF37]/80">{alphaValue}</span>
          <span className="text-white/30"> = </span>
          <span className={calculations.patternValue === 625284 ? 'text-[#D4AF37]/90' : 'text-red-400/70'}>
        ...
        {calculations.isCorrectFormula && (
          <motion.div
            ...
            className="inline-flex items-center gap-2 text-[#D4AF37]/70 text-sm"
          >
      ...
      <div className="px-6 md:px-8 space-y-7 mb-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-2.5">
            <label className="text-sm font-medium flex items-center gap-2">
              <span className="text-[#D4AF37]/80">Block Height</span>
              <button
                onClick={() => setShowExplanation(showExplanation === 'block' ? null : 'block')}
                className="text-white/30 hover:text-[#D4AF37]/70 transition-colors"
              >
                <Info className="h-3.5 w-3.5" />
              </button>
            </label>
            <div className="flex items-center gap-2">
              <span className="font-mono text-sm text-[#D4AF37]/80">{blockHeight}</span>
              {blockHeight === CORRECT_BLOCK ? (
                <Check className="h-3.5 w-3.5 text-[#D4AF37]/60" />
              ) : (
          ...
          <div className="flex justify-between text-xs text-white/25 mt-1.5">
            <span>1</span>
            <span className={blockHeight === CORRECT_BLOCK ? 'text-[#D4AF37]/60 font-medium' : ''}>
              283 (correct)
            </span>
            <span>500</span>
          </div>
          {showExplanation === 'block' && (
            <motion.p
              ...
              className="text-xs text-white/40 mt-3 p-3 bg-white/[0.02] border border-white/[0.05] leading-relaxed"
            >
              {explanations.block}
            </motion.p>
          )}
        ...
        <div>
          <div className="flex items-center justify-between mb-2.5">
            <label className="text-sm font-medium flex items-center gap-2">
              <span className="text-[#D4AF37]/80">Prime Value (squared)</span>
              <button
          ...
          <div className="flex justify-between text-xs text-white/25 mt-1.5">
            <span>2</span>
            <span className={primeValue === CORRECT_PRIME ? 'text-[#D4AF37]/60 font-medium' : ''}>
              47 (correct)
            </span>
            <span>100</span>
          </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2.5">
            <label className="text-sm font-medium flex items-center gap-2">
              <span className="text-[#D4AF37]/80">Alpha Constant (&alpha;)</span>
              <button
            ...
            <div className="flex items-center gap-2">
              <span className="font-mono text-sm text-[#D4AF37]/80">{alphaValue}</span>
              {alphaValue === CORRECT_ALPHA ? (
                <Check className="h-3.5 w-3.5 text-[#D4AF37]/60" />
              ) : (
                <X className="h-3.5 w-3.5 text-red-400/40" />
          ...
          <div className="flex justify-between text-xs text-white/25 mt-1.5">
            <span>1</span>
            <span className={alphaValue === CORRECT_ALPHA ? 'text-[#D4AF37]/60 font-medium' : ''}>
              137 (correct)
            </span>
            <span>200</span>
          </div>
        ...
        <div className="space-y-2.5">
          {calculations.steps.map((step, index) => (
            <motion.div
              ...
              className="flex items-center justify-between p-3.5 bg-white/[0.03] border border-white/[0.08]"
            >
              <div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                {step.isCorrect !== undefined && (
                  step.isCorrect ? (
                    <Check className="h-4 w-4 text-[#D4AF37]/60" />
                  ) : (
      ...
      {calculations.row === 21 && calculations.col === 4 && (
        <motion.div
          ...
          className="mx-6 md:mx-8 mt-5 p-4 bg-[#D4AF37]/[0.04] border border-green-500/[0.12]"
        >
          <div className="flex items-start gap-3">
            <Check className="h-4 w-4 text-[#D4AF37]/70 mt-0.5 shrink-0" />
            <div>
              <div className="text-sm font-medium text-[#D4AF37]/80">Row 21, Column 4 -- Bitcoin Input Layer</div>
      ...
      <div className="px-6 md:px-8 py-6 mt-2 text-center">
        <button
          ...
          className="px-5 py-2.5 text-sm bg-white/[0.06] border border-white/[0.10] text-white/60 hover:bg-white/[0.10] hover:text-white/80 transition-all duration-300"
        >
