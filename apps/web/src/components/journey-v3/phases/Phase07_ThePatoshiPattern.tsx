    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-10 gap-1">
        {PATOSHI_BLOCKS.map((block) => (
          <motion.button
            key={block.height}
            className={`aspect-square rounded cursor-pointer transition-all relative ${
              block.isPatoshi
      ...
      {hoveredBlock !== null && !selectedBlock && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="p-2 rounded bg-white/5 text-sm text-center"
        >
          Block #{hoveredBlock} -{' '}
          {PATOSHI_BLOCKS[hoveredBlock - 1]?.isPatoshi ? (
            <span className="text-orange-400">Patoshi miner</span>
          ) : (
      ...
      <AnimatePresence>
        {selectedBlock && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="p-4 rounded-xl bg-gradient-to-b from-orange-950/30 to-black/50 border border-orange-500/30"
          >
  ...
  return (
    ...
    >
      <div ref={ref} className="space-y-8">
        {/* Introduction */}
        <motion.div
          className="p-6 md:p-8 rounded-2xl bg-gradient-to-b from-orange-950/20 to-black/50 border border-orange-500/20"
        ...
        >
          <div className="space-y-3 text-sm text-white/70">
            <p>
              The Patoshi pattern analysis is <span className="text-yellow-400">Tier 2 evidence</span>{' '}
